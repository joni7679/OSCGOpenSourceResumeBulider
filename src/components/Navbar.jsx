...
function Navbar() {
    ...
    return (
        <>
            <ToastContainer />
            <nav className="w-full mx-auto px-4 sm:px-6 lg:px-8  bg-[#E6EFFC] backdrop-blur-md shadow-xl sticky top-0 z-50  transition-all duration-300">
                    ...
                    <div className="flex justify-between items-center h-16 lg:h-20">
                        ...
                        <div className="hidden lg:flex items-center gap-3">
                            {!user ? (
                                ...
                                <Link
                                    to={`/dashboard`}
                                    className="bg-blue-500  text-white px-5 py-2 rounded-md shadow ">
                                    Go To Dashboard
                                </Link>
                            )}
                        </div>
            ...
            <div
                ref={menuRef}
                className={`fixed top-[10%] right-0 w-2/3 sm:w-1/2 h-full bg-[#1b1f23]/95 backdrop-blur-lg shadow-2xl z-50 lg:hidden flex flex-col items-start p-8 gap-8 rounded  transition-transform duration-500 ${menuOpen ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none'}`}
                role="menu"
                ...
                {!user ? (
                    <>
                        ...
                        <Link
                            to="/login"
                            onClick={() => setMenuOpen(false)}
                            className=" bg-blue-500  text-white px-4 py-2 rounded-md shadow w-full text-center font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-orange-400"
                            role="menuitem">
                            Login
                        </Link>
                    </>
                    ...
                    <Link
                        to="/dashboard"
                        onClick={() => setMenuOpen(false)}
                        className="bg-blue-800  text-white px-4 py-2 rounded-md shadow w-full text-center font-semibold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-400"
                        role="menuitem">
                        Go To Dashboard
                    </Link>
                )}
